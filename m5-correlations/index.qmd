...
title-slide-attributes:
  data-background-image: "plot/correlation-title-page.png"
...
## working directory
setwd(here::here("m5-correlations"))
...
$$
r = \frac{\Sigma{xy} - \frac{(\Sigma{x}) (\Sigma{y})}{n}}{\sqrt{[\Sigma x^2 - \frac{(\Sigma x)^2}{n}][\Sigma x^2 - \frac{(\Sigma y)^2}{n}]}}
$$
...
r <- with(sample_comp_data,
          (sum_xy - (sum_x * sum_y) / nrow(sample_data)) /
            sqrt((sum_x2 - (sum_x^2) / nrow(sample_data)) *
                   (sum_y2 - (sum_y^2) / nrow(sample_data)))
)
...
## correlation test
cor.test(sample_data$x, sample_data$y, method = "pearson")
```
:::
:::::
...
```{r}
knitr::include_graphics("plot/econ115a-hex-logo.png")
```